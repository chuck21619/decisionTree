...
def train_model(x_player, y_player, le_input_players_input, le_target_players_input, x_deck, y_deck, le_input_decks_input, le_target_decks_input):
    ...
    """
    global model_player, model_deck, le_input_players, le_target_players, le_input_decks, le_target_decks
...
def model_predict(game_input):
    ...
    allowed_players = set(game_input.keys())  # Get the players from the game input
    allowed_decks = set(game_input.values())  # Get the decks from the game input
    ...
    final_prediction = model_player.predict(combined_features)  # Using player model for the final prediction
